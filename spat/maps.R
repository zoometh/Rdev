...
huelva.df <- dbGetQuery(con, steles.bouclier.sqll)
...
ggmap::ggmap(stamenbck) +
  geom_sf(data = steles.bouclier, inherit.aes = FALSE) +
  coord_sf(crs = 3857) +
  coord_map(projection = 3857)
  geom_sf(data = sites, inherit.aes = FALSE)
...
plot(st_transform(steles.bouclier, 3857)[1], col = "black", fill = "black", bgMap = stamenbck)
