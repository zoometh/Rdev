...
df <- data.frame(data = c("steles.bouclier",
                 ...
                 sqll = c(paste0 (sqll.wkt, "famille LIKE 'stele bouclier'"),
                          ...
                          paste0 (sqll.wkt, "site LIKE 'Ategua' LIMIT 1")
                 )
                )
#
#
...
ategua <- to.mercator(sqll.me(data = "ategua", df))
kville <- to.mercator(sqll.me(data = "kville", df))
...
roi.extent <- st_union(huelva, steles.bouclier)
roi.extent <- st_transform(xxx, crs = 4326)
roi <- as.numeric(st_bbox(roi.extent))
