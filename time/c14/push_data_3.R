...
## - - - - - - - - - - - - - - - - - - - - - - - 
## Prepare data for Create Rshiny app data
...
path.data <- paste0(gt.master,"neonet/")
...
# fich <- "NeoNet_rvTH.xlsx" # whith special characters + Nicco data
fich <- "NeoNet_rvTH_8.xlsx" # whith special characters + Nicco data
...
if(join.c14data.and.c14ref){
  ...
  df.tot <- df.tot[,c("SiteName", "Country", "Period", "PhaseCode",
                      "LabCode", "C14Age", "C14SD", "Material", "MaterialSpecies",
                      "tpq", "taq", 
