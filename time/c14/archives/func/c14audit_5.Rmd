...
# SELECT ATLANTIC
NEONET <- read.xlsx(paste0(c14.path, "docs/data/", neonet.bd), 
                    skipEmptyRows = T)
...
1. [Merge NEONET Portugal with other DBs](#addBDs)
2. [Create the ROI map](#roi)
3. [Select InTime and InSpace](#inST)
4. [Create a Shiny map](#spat)
...
BDA_sub$BD <- "BDA"
# AGRICHANGE - - - - - - - - - - - - - - - - - - - - -
AGRICHANGE <- get_agrichange()
# c14.BD.equ$GET
AGRICHANGE_sub <- AGRICHANGE[, na.omit(c14.BD.equ$AGRICHANGE)]
fields.to.add <- c14.BD.equ[is.na(c14.BD.equ$AGRICHANGE), "NEONET"]
names(AGRICHANGE_sub) <- c14.BD.equ$NEONET[!(c14.BD.equ$NEONET %in% fields.to.add)]
AGRICHANGE_sub[, fields.to.add] <- NA
AGRICHANGE_sub$BD <- "AGRI"
# NEONET NICCO - - - - - - - - - - - - - - - - - - - - 
MED <- read.xlsx(paste0(c14.path, "docs/data/","NeoNet_rvTH_8.xlsx"), 
                          skipEmptyRows = T)
MED_sub <- MED[, c14.BD.equ$NEONET]
# fields.to.add <- c14.BD.equ[is.na(c14.BD.equ$AGRICHANGE), "NEONET"]
# names(AGRICHANGE_sub) <- c14.BD.equ$NEONET[!(c14.BD.equ$NEONET %in% fields.to.add)]
# AGRICHANGE_sub[, fields.to.add] <- NA
MED_sub$BD <- "MED"
...
NEONET$BD <- "POR"
# join
mergedDB <- rbind(NEONET, BDA_sub, AGRICHANGE_sub, MED_sub)
```
...
C14.filter <- function(NEONET, 
  ...
  if(filtered){
    NEONET.filtered <- NEONET.df[!is.na(NEONET.df$inSpace) & !is.na(NEONET.df$inTime), ]
...
n.BDs <- length(unique(mergedDB$BD))
brewer.pal(n.BDs,"Set1")
df.colors <- data.frame(BD = unique(mergedDB$BD),
...
mergedDB$lbl <- paste0("<b>", mergedDB$SiteName," - ", mergedDB$LabCode, "</b><br>",
                      ...
                      mergedDB$Period," - ",  mergedDB$PhaseCode, "<br>",
                       "<b>", mergedDB$BD,"</b>")
ws_roi.shp.sp <- as(ws_roi.shp, "Spatial")
# ws_atl.shp.sp <- as(ws_atl.shp, "Spatial")
map <- leaflet(data = mergedDB, width = "70%", height = "600px") %>%
  addTiles(group = 'OSM') %>%
...
map
library(htmlwidgets)
saveWidget(map, file=paste0("C:/Rprojects/neonet/doc/img/neonet_atl.html"))
