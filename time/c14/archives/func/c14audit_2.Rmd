...
# neonet.bd <- "NeoNet_rvTH.xlsx"
neonet.bd <- "NeoNet_rvTH_9.xlsx"
# neonet.bd <- "c14_dataset.tsv"
...
C14.filter <- function(NEONET, 
  ...
  if(nrow(NEONET.noXY) > 0){
    ...
    write.xlsx(x = NEONET.noXY,
               file = paste0(c14.path.data, out.file.noXY),
               col.names=TRUE)
  ...
  write.xlsx(x = NEONET.df,
             file = paste0(c14.path.data, out.file),
             col.names=TRUE)
  if(filtered){
...
NEONET <- read.xlsx(paste0(c14.path.data, neonet.bd)) # read
...
# sf class
ws_roi.extent <- extent(ws_roi.shp)
...
# box <- c(xmin = xmin.sel - 0.5,
#          ymin = ymin.sel - 0.5,
#          xmax = xmax.sel + 0.5,
#          ymax = ymax.sel + 0.5)
# sp.fond.select <- crop(bck_admin.shp, extent((xmin.sel - 0.5), 
#                                              (xmax.sel + 0.5), # pour la légende
#                                              (ymin.sel - 0.5), 
#                                              (ymax.sel + 0.5)))
# # fond.sf <- st_as_sf(sp.fond.select)
# sp.fond.select <- st_crop(bck_admin.shp, 
#                           xmin = xmin.sel - 0.5,
#                           ymin = ymin.sel - 0.5,
#                           xmax = xmax.sel + 0.5,
#                           ymax = ymax.sel + 0.5)
...
Check the completeness of `r neonet.bd`, comparing it with other databases
...
gdistr.BP <- ggplot(data = NEONET.clean, aes(x = C14BP))+  
  ...
  theme_bw()
# BC - - - - - - - - - - - - - - - - - - - - - - - - - -
# NEONET.clean$tpq <- NEONET.clean$taq <- NA
# for(i in 1:nrow(NEONET.clean)){
#   if(i %% 100 == 0) print(paste0(as.character(i),"/", as.character(nrow(NEONET.clean))))
#   ages1 <- BchronCalibrate(ages = NEONET.clean[i,"C14BP"],
#                            ageSds = NEONET.clean[i,"C14SD"],
#                            calCurves = intCal,
#                            ids = 'Date1')
#   NEONET.clean[i,"tpq"] <- -(min(ages1$Date1$ageGrid) - 1950)  
#   NEONET.clean[i,"taq"] <- -(max(ages1$Date1$ageGrid) - 1950) 
# }
