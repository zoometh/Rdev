...
[c14bazAAR](https://github.com/ropensci/c14bazAAR) is a polyvalent package, allowing numerous operations. Using this package with [Leaflet](https://rstudio.github.io/leaflet/) allow to create interactive map of radiocarbon dates coming from different databases, starting -- for example -- from the "BDA" database [@Perrin21]
...
<!-- # DBs <- DBs[! DBs %in% c("adrac","austarch")]# sic -->
<!-- # choices = "katsianis", selected = "katsianis", width = "25%"), -->

```{r echo = TRUE, warning=FALSE}
library(shiny)
library(c14bazAAR)
library(leaflet)
DBs.list.url <-'https://raw.githubusercontent.com/ropensci/c14bazAAR/master/data-raw/db_info_table.csv'
DBs.list <- read.csv(DBs.list.url)
DBs <- sort(DBs.list$db)
ui <- fluidPage(selectInput("c14.db", label = "c14 databases",
                            choices = DBs, selected = "bda", width = "25%"),
...
```{r echo = TRUE, warning=FALSE}
url.root <- 'https://archaeologydataservice.ac.uk/catalogue/adsdata/arch-282-12/'
dendro.list.url <- paste0(url.root, 'dissemination/csv/VA_Dend_2021_ADS-IND_FUL_TO51.csv&hs=true')
dendro.list <- read.csv(dendro.list.url, stringsAsFactors = F)
dendro <- dendro.list[nchar(dendro.list$Grid.letter) == 2, ]
dendro <- dendro[nchar(dendro$Easting) == 3, ]
dendro <- dendro[nchar(dendro$Northing) == 3, ]
dendro <- dendro[!is.na(dendro$Year_short), ]
dendro$GEO <- paste0(dendro$Grid.letter, dendro$Easting, dendro$Northing)
