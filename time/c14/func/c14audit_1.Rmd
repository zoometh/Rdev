...
C14.filter <- function(tpq.bp = tpq.bp, taq.bp = taq.bp, roi = ws_roi.shp){
  # tag inTime & inSpace == 1 when fits
  # == tag by ROI and chronology
  NEONET <- read.xlsx(paste0(c14.path.data, neonet.bd))
  # by time
  # time.flag <- paste0(tpq.bp, "_", taq.bp, "BP")
  NEONET$inTime <- ifelse(NEONET$C14BP <= tpq.bp & NEONET$C14BP >= taq.bp, 1, NA)
  # by ROI
  xy <- list(longitude = as.numeric(NEONET$Longitude),
             latitude = as.numeric(NEONET$Latitude))
  NEONET.sp <- SpatialPointsDataFrame(coords = xy,
                                      data = NEONET,
                                      proj4string = CRS(WGS84))
  
  # coordinates(NEONET) <- ~ NEONET$Longitude + NEONET$Latitude # get coords for RTB
  NEONET.sf <- st_as_sf(NEONET.sp)
  # st_crs(BDA.select) <- "+init=epsg:4326"
  # NEONET.sf$Longitude <- NEONET.sf$Latitude <- NA
  # BDA.select.sp <- as(NEONET.sf, "Spatial")
  # # recover
  ...
  NEONET.sf <- NEONET.sf %>% mutate(
    ...
    # inSpace = if_else(is.na(intersection), 0, 1)
  ) 
  # save 
  ...
  write.xlsx(x = NEONET.df,
             ...
             col.names=TRUE)
  # NEONET.filtered <- as.data.frame(NEONET.sf)
...
Check the completness of `r neonet.bd`, comparing it with other databases
...
gdistr <- ggplot(data = NEONET.clean, aes(x = C14BP))+  
  ...
  geom_vline(xintercept = tpa.bp, linetype="dotted", color = "blue") + 
