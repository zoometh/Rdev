...
app.page <- paste0('<a href=',shQuote(webpage.app),"\ target=\"_blank\"",'>https://zoometh.github.io/C14/neonet/</a>')
# GitHub repo
devpage.app <- "https://github.com/zoometh/C14/tree/main/neonet"
...
app.credits <- HTML(paste0(' <b> IT developments: </b> ',
                           ...
                           '> Niccolo Mazzucco </a>: niccolo.mazzucco@unipi.it </li>',
                           '</ul>'))
app.website <- HTML(paste0(' <b> App & DB documentation: </b> ',
                           '<ul>',
                           '<li>', app.page,
                           '</li>',
                           '</ul>'))
app.devsite <- HTML(paste0(' <b> App dev: </b> ',
                           ...
                           '</li>',
                           '</ul>'))
all.credits <- paste0(data.credits, "<br>",
                      app.credits, "<br>",
                      app.website, "<br>",
                      app.devsite
)
    ...
    if (nrow(filteredData()) > 0) {
      ...
      proxy.sites %>% 
        ...
        addLabelOnlyMarkers(0,
                            45,
                            label=paste(legende_ord$colors[6], legende_ord$Period[6]),
                            labelOptions = labelOptions(noHide = T, direction = 'top', textOnly = T, textsize = "15px"))  %>%
        addLegend("bottomleft",
                  colors = legende_ord$colors,
                  labels= legende_ord$Period,
