...
c14bibtex.url <- 'references_OK_7.bib'
...
# df.tot <- read.table("c14_dataset.tsv", sep = "\t", encoding = "utf-8")
...
app.page <- HTML(paste0('<a href=',shQuote(webpage.app),"\ target=\"_blank\"",'><b> NeoNet app </b>website</a>'))
...
server <- function(input, output, session) {
  ...
  # cell edition of the filtered table 'tbl'
  ...
  # cell edition of the small filtered table
  ...
  # - - - - - - - - - - - - - - - - - - - - - - -
  # to select by polygon see selection by polygon
