---
title: "Use of <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-prg-r.png' style='height: 100px;vertical-align: middle;'> at the<br> School of Archaeology"
# subtitle: "Create beautiful interactive slide decks with Reveal.js"
author: "Thomas Huet (University of Oxford, EAMENA project)"
title-slide-attributes:
    data-background-image: "https://raw.githubusercontent.com/zoometh/thomashuet/master/img/inst-uni-oxford-research-areas.png"
    data-background-size: "80%"
    data-background-opacity: "0.3"
format:
  revealjs: 
    css: https://zoometh.github.io/thomashuet/css/quarto.css
    code-fold: true
    code-summary: "<small>Code</small>"
    slide-number: true
    chalkboard: true
    scrollable: true
    preview-links: auto
    reference-location: document
    footnotes-hover: true
    # self-contained: true
    # embed-resources: true
    logo: https://raw.githubusercontent.com/zoometh/thomashuet/master/img/inst-uni-eamena.png
    footer: "<em>Workshop on the use of 'R' at the School of Archaeology</em> <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/inst-uni-oxford.png' style='height: 50px;vertical-align: middle;'> Univesity of Oxford, School of Archaeology, 5 December 2023 - Thomas Huet"
    margin: 0
    width: 1200
---


## Why <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-prg-r.png' style='height: 50px;vertical-align: middle;'> ?

<span class="captiontext">"*anything that can be automated, should be automated*" -- Hadley Wickham</span>

```{=html}
<iframe width="1200" height="500" src="https://www.tiobe.com/tiobe-index/" title="research-themes"></iframe>
```

## {}

::: {.panel-tabset}

### resources

| Host 	| URL 	|
|------	|-----	|
| ![](https://raw.githubusercontent.com/zoometh/thomashuet/master/img/src-archeo-prog-openarchaeo.png){width=350px} 	| <https://open-archaeo.info/>    	|
| ![](https://raw.githubusercontent.com/zoometh/thomashuet/master/img/src-archeo-prog-ssl-map-comput.png){width=350px}  	| <https://sslarch.github.io/>    	|

### `open-archaeo`

```{=html}
<iframe width="1200" height="500" src="https://open-archaeo.info/" title="research-themes"></iframe>
```


### `SSLA`

```{=html}
<iframe width="1100" height="500" src="https://sslarch.github.io/MapofComputationalArchaeology/" title="research-themes"></iframe>
```

:::


## Research areas

```{r }
#| echo: true
#| eval: false
#| code-fold: true

devtools::source_url("https://raw.githubusercontent.com/zoometh/Rdev/master/text/soa_cloudword_1.R")
research.soa(soa.url = 'https://www.arch.ox.ac.uk/', 
             soa.research.areas = list('bioarchaeology', 'chronology', 'eurasian-prehistory', 'historical-and-classical', 'materials-and-technology', 'palaeolithic'))
```

![](https://raw.githubusercontent.com/zoometh/thomashuet/master/img/inst-uni-oxford-research-areas-large.png){width=350px} 


## Bioarchaeology <img src='https://www.arch.ox.ac.uk/sites/default/files/archit/images/media/bannergrasses1.jpg' style='height: 60px;vertical-align: middle;'>

<div class="row">
<div class="column33">![](https://raw.githubusercontent.com/zoometh/Rdev/master/text/img/bioarchaeology.png){height=500px}</div>
<div class="column66">
```{=html}
<iframe width="780" height="500" src="https://open-archaeo.info/tags/biological-anthropology/" title="research-themes"></iframe>
```
</div>
</div>



## Chronology <img src='https://www.arch.ox.ac.uk/sites/default/files/archit/images/media/bannerlab1jpg.jpg' style='height: 60px;vertical-align: middle;'>

<div class="row">
<div class="column33">![](https://raw.githubusercontent.com/zoometh/Rdev/master/text/img/chronology.png){height=500px}</div>
<div class="column66">
```{=html}
<iframe width="780" height="500" src="https://open-archaeo.info/tags/chronological-modelling/" title="research-themes"></iframe>
```
</div>
</div>

## Eurasian Prehistory <img src='https://www.arch.ox.ac.uk/sites/default/files/archit/images/media/bannercave.jpg' style='height: 60px;vertical-align: middle;'>

<div class="row">
<div class="column33">![](https://raw.githubusercontent.com/zoometh/Rdev/master/text/img/eurasian-prehistory.png){height=500px}</div>
<div class="column66">
```{=html}
<iframe width="780" height="500" src="https://open-archaeo.info/tags/palaeoenvironment-and-palaeobotany/" title="research-themes"></iframe>
```
</div>
</div>

## Historical and Classical <img src='https://www.arch.ox.ac.uk/sites/default/files/archit/images/media/bannerdig1.jpg' style='height: 60px;vertical-align: middle;'>

<div class="row">
<div class="column33">![](https://raw.githubusercontent.com/zoometh/Rdev/master/text/img/historical-and-classical.png){height=500px}</div>
<div class="column66">
```{=html}
<iframe width="780" height="500" src="https://open-archaeo.info/tags/shape-recognition/" title="research-themes"></iframe>
```
</div>
</div>

## Materials and Technology <img src='https://www.arch.ox.ac.uk/sites/default/files/archit/images/media/bannermaterials.jpg' style='height: 60px;vertical-align: middle;'>

<div class="row">
<div class="column33">![](https://raw.githubusercontent.com/zoometh/Rdev/master/text/img/materials-and-technology.png){height=500px}</div>
<div class="column66">
```{=html}
<iframe width="780" height="500" src="https://open-archaeo.info/tags/instrumental-neutron-activation-analysis/" title="research-themes"></iframe>
```
</div>
</div>

## Palaeolithic <img src='https://www.arch.ox.ac.uk/sites/default/files/archit/images/media/bannerpalaeo.jpg' style='height: 60px;vertical-align: middle;'>

<div class="row">
<div class="column33">![](https://raw.githubusercontent.com/zoometh/Rdev/master/text/img/palaeolithic.png){height=500px}</div>
<div class="column66">
```{=html}
<iframe width="780" height="500" src="https://open-archaeo.info/tags/cultural-evolution/" title="research-themes"></iframe>
```
</div>
</div>


## {}

::: {.panel-tabset}

### JPG/PNG

How: <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/r-pkg-ggplot.png' style='height: 50px;vertical-align: middle;'>, Where: <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-github.png' style='height: 50px;vertical-align: middle;'>

![](https://raw.githubusercontent.com/eamena-project/eamenaR/master/results/map_path_profile.png){height=500px}

### HTML

How: <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-prg-js-geo-leaflet.png' style='height: 50px;vertical-align: middle;'> <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-prg-js-plotly.png' style='height: 50px;vertical-align: middle;'>, Where: <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-github.png' style='height: 50px;vertical-align: middle;'>

```{=html}
<iframe width="780" height="500" src="https://eamena-project.github.io/reveal.js/caravanserail#/1" title="research-themes"></iframe>
```


### JavaScript

How: <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-prg-js-geo-leaflet.png' style='height: 50px;vertical-align: middle;'> <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-prg-js-plotly.png' style='height: 50px;vertical-align: middle;'> + <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-prg-r-pkg-shiny.png' style='height: 50px;vertical-align: middle;'>, Interactive Data Network <https://idn.it.ox.ac.uk/home>

```{=html}
<iframe width="1100" height="700" src="https://trainingidn.shinyapps.io/togo/" title="research-themes"></iframe>
```


:::

## At the University of Oxford <img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/inst-uni-oxford.png' style='height: 50px;vertical-align: middle;'>



